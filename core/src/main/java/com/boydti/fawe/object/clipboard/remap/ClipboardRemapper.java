...
public class ClipboardRemapper {
        ...
        if (from == RemapPlatform.PC) {
            ...
            if (state != null) {
                ...
                try {
                    ...
                    switch (name) {
                        case "spruce_boat":
                            return new BaseItem(333, (short) 1);
                        case "birch_boat":
                            return new BaseItem(333, (short) 2);
                        case "jungle_boat":
                            return new BaseItem(333, (short) 3);
                        case "acacia_boat":
                            return new BaseItem(333, (short) 4);
                        case "dark_oak_boat":
                            return new BaseItem(333, (short) 5);
                        case "water_bucket":
                            return new BaseItem(325, (short) 8);
                        case "lava_bucket":
                            return new BaseItem(325, (short) 10);
                        case "milk_bucket":
                            return new BaseItem(325, (short) 1);
                        case "tipped_arrow":
                        case "spectral_arrow":
    ...
    private HashMap<BaseBlock, BaseBlock> getPEtoPCMappings() {
        ...
        { // beetroot
            ...
            for (int data = 3; data < 16; data++)
                mapPEtoPC.putIfAbsent(new BaseBlock(244, data), new BaseBlock(207, data));
        }
...
//            mapPEtoPC.put(new BaseBlock(id,12), new BaseBlock(id,13));
        ...
        for (int id : new int[]{96, 167}) { // trapdoor
            ...
            for (int data = 0; data < 4; data++)
                mapPEtoPC.putIfAbsent(new BaseBlock(id, data), new BaseBlock(id, 3 - data));
            for (int data = 4; data < 12; data++)
                mapPEtoPC.putIfAbsent(new BaseBlock(id, data), new BaseBlock(id, 15 - data));
            for (int data = 12; data < 16; data++)
                mapPEtoPC.putIfAbsent(new BaseBlock(id, data), new BaseBlock(id, 27 - data));
        }
